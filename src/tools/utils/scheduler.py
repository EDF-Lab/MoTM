...
def get_scheduler(cfg, optimizer):
    ...
    if name == 'CosineAnnealing':
        ...
        scheduler = scheduler_func(optimizer, T_0 = cfg.T_0, T_mult = cfg.T_mult, eta_min = cfg.eta_min)
    elif name == 'CosineWithRestart':
        scheduler_func = get_cosine_with_hard_restarts_schedule_with_warmup
        scheduler = scheduler_func(optimizer, num_warmup_steps=cfg.num_warmup_steps, num_training_steps=cfg.T_max, num_cycles=1, last_epoch=-1)
    else:
